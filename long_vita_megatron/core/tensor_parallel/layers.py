...
def linear_with_frozen_weight(
    ...
    if logit_mask is not None:
        ...
        b = input.size(1)
        c = input.size(2)
        input = torch.masked_select(input, logit_mask.transpose(0, 1).unsqueeze(2)).reshape(-1, b, c)
...
class LinearWithGradAccumulationAndAsyncCommunication(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.save_for_backward(input, weight, logit_mask)
        ...
        if logit_mask is not None:
            s = total_input.size(0)
            b = total_input.size(1)
            c = total_input.size(2)
            # [s b c]
            total_input = torch.masked_select(total_input, logit_mask.transpose(0, 1).unsqueeze(2)).reshape(-1, b, c)
    ...
    @custom_bwd
    def backward(ctx, grad_output):
        input, weight, logit_mask = ctx.saved_tensors
        ...
        if logit_mask is not None:
            ...
            c = grad_input.size(2)
            # [s b c]
            grad_input = torch.zeros([s, b, c], dtype=grad_input.dtype, device=grad_input.device).masked_scatter(logit_mask.transpose(0, 1).unsqueeze(2), grad_input)
...
class ColumnParallelLinear(torch.nn.Module):
    ...
    def forward(self, input_: torch.Tensor, weight: Optional[torch.Tensor] = None, logit_mask = None):
