...
class GPTVLModel(LanguageModule):
    ...
    def __init__(
        ...
        self.unused = torch.nn.Parameter(0.01 * torch.ones(config.hidden_size))
    ...
    def forward(
        ...
        if hasattr(inference_params, 'logit_mask') and inference_params.logit_mask is not None:
            logit_mask = inference_params.logit_mask
            # print("Setting logit_mask to inference_params.logit_mask......")

        if hasattr(inference_params, "use_kv_cache") and not inference_params.use_kv_cache:
        ...
        if self.position_embedding_type == 'rope':
            rotary_seq_len = self.rotary_pos_emb.get_rotary_seq_len(
                ...
                # None, self.decoder, decoder_input, self.config
            )
...
def _load_state_dict_hook_ignore_param_names(
    ...
    for key in list(incompatible_keys.missing_keys):
        ...
        if flag:
            ...
            print(f"Ignore missing {key}", flush=True)
