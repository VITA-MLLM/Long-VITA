...
def set_mlp_state(args, layer, hf_layer):
    ...
    for idx in range(args.num_experts):
        w1 = getattr(mlp.experts.local_experts, f"{idx}").linear_fc1.weight.data.copy_
        w2 = getattr(mlp.experts.local_experts, f"{idx}").linear_fc2.weight.data.copy_

        hf_w1w3_weight = torch.cat([
            getattr(hf_mlp.experts, f"{idx}").w1.weight,
            getattr(hf_mlp.experts, f"{idx}").w3.weight,
        ], dim=0)
