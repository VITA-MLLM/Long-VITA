...
def save_model_checkpoint(queue, args):
    ...
    try:
        import modellink
        import long_vita_modellink.megatron_adaptor
    ...
    sys.argv = [
        ...
        '--sequence-parallel',
        '--bf16',
        '--num-layer-list', str(args.num_layer_list),
        '--use-mcore-models',
    ]
    ...
    for ep_rank in range(args.target_expert_parallel_size):
        for tp_rank in range(args.target_tensor_parallel_size):
            models[ep_rank][tp_rank].embedding.word_embeddings.weight.data.copy_(out_word_embed[tp_rank])
            if pos_embed is not None:
                models[ep_rank][tp_rank].embedding.position_embeddings.weight.data.copy_(pos_embed)
            else:
                if hasattr(models[ep_rank][tp_rank].embedding, 'position_embeddings'):
    ...
    for pp_rank in range(args.target_pipeline_parallel_size):
        ...
        for layer_id, _ in enumerate(models[0][0].decoder.layers):
            ...
            mlp_gate_weight = msg.pop("router weight")

            if md.linear_bias:
                qkv_bias = torch.chunk(msg.pop("qkv bias"), args.target_tensor_parallel_size, dim=0)
                mlp_gate_bias = msg.pop("router bias")
            ...
            print("mlp_l0_weight", mlp_l0_weight.size())
            print("mlp_l1_weight", mlp_l1_weight.size())

            for ep_rank in range(args.target_expert_parallel_size):
                for tp_rank in range(args.target_tensor_parallel_size):
                    layer_chunk = models[ep_rank][tp_rank].decoder.layers[layer_id]
                    for local_expert_idx in range(num_local_experts):
                        w0_weight = getattr(
                            layer_chunk.mlp.experts.local_experts,
                            f"{local_expert_idx}").linear_fc1.weight.data.copy_
                        w1_weight = getattr(
                            layer_chunk.mlp.experts.local_experts,
                            f"{local_expert_idx}").linear_fc2.weight.data.copy_

                        w0_weight(mlp_l0_weight[ep_rank][tp_rank][local_expert_idx])
                        w1_weight(mlp_l1_weight[ep_rank][tp_rank][local_expert_idx])

                        assert layer_chunk.mlp.experts.local_experts[local_expert_idx].linear_fc1.weight.size() == mlp_l0_weight[ep_rank][tp_rank][local_expert_idx].size()
                        assert layer_chunk.mlp.experts.local_experts[local_expert_idx].linear_fc2.weight.size() == mlp_l1_weight[ep_rank][tp_rank][local_expert_idx].size()
            ...
            for ep_rank in range(args.target_expert_parallel_size):
                for tp_rank in range(args.target_tensor_parallel_size):
                    ...
                    layer_chunk.self_attention.linear_proj.weight.data.copy_(dense_weight[tp_rank])
                    layer_chunk.pre_mlp_layernorm.weight.data.copy_(post_norm_weight)
                    layer_chunk.mlp.router.weight.data.copy_(mlp_gate_weight)

                    assert layer_chunk.input_layernorm.weight.size() == input_norm_weight.size()
                    assert layer_chunk.self_attention.linear_qkv.weight.size() == qkv_weight[tp_rank].size()
                    assert layer_chunk.self_attention.linear_proj.weight.size() == dense_weight[tp_rank].size()
                    assert layer_chunk.pre_mlp_layernorm.weight.size() == post_norm_weight.size()
                    assert layer_chunk.mlp.router.weight.size() == mlp_gate_weight.size()
        ...
        if post_process:
            ...
            for ep_rank in range(args.target_expert_parallel_size):
                for tp_rank in range(args.target_tensor_parallel_size):
                    models[ep_rank][tp_rank].decoder.final_layernorm.weight.data.copy_(final_norm_weight)
                    assert models[ep_rank][tp_rank].decoder.final_layernorm.weight.size() == final_norm_weight.size()
                    if md.norm_has_bias:
                        models[ep_rank][tp_rank].decoder.final_layernorm.bias.data.copy_(final_norm_bias)
            ...
            if md.output_layer:
                ...
                if not hasattr(models[0][0], 'output_layer'):
                ...
                for ep_rank in range(args.target_expert_parallel_size):
                    for tp_rank in range(args.target_tensor_parallel_size):
                        models[ep_rank][tp_rank].output_layer.weight.data.copy_(
                            output_layer_weight[tp_rank])
                        assert models[ep_rank][tp_rank].output_layer.weight.size() == output_layer_weight[tp_rank].size()
                del output_layer_weight
                check_message(msg)

            msg = queue_get()
            if msg != "done" and msg["name"] == "pooler":
                if not hasattr(models[0][0], 'pooler'):
                ...
                for ep_rank in range(args.target_expert_parallel_size):
                    for tp_rank in range(args.target_tensor_parallel_size):
                        models[ep_rank][tp_rank].pooler.dense.weight.data.copy_(pooler_weight)
                        models[ep_rank][tp_rank].pooler.dense.bias.data.copy_(pooler_bias)
                del pooler_weight
                del pooler_bias
                check_message(msg)
                msg = queue_get()
            ...
            if msg != "done" and msg["name"] == "lm head":
                ...
                for ep_rank in range(args.target_expert_parallel_size):
                    for tp_rank in range(args.target_tensor_parallel_size):
                        ...
                        assert models[ep_rank][tp_rank].lm_head.dense.weight.size() == lm_head_dense_weight.size()
                        assert models[ep_rank][tp_rank].lm_head.dense.bias.size() == lm_head_dense_bias.size()
                        assert models[ep_rank][tp_rank].lm_head.norm.weight.size() == lm_head_norm_weight.size()
                        if md.norm_has_bias:
