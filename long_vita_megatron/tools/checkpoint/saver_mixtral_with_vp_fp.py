...
def save_model_checkpoint(queue, args):
    ...
    sys.argv = [
        ...
        '--first-pipeline-num-layers', str(args.first_pipeline_num_layers),
    ...
    for pp_rank in range(args.target_pipeline_parallel_size):
        ...
        for vpp_rank in range(vp_size):
            ...
            for layer_id in range(len(models[0][0].language_model.encoder.layers) // vp_size):
                if args.num_layers_per_virtual_pipeline_stage and args.save_model_type == 'megatron':
                    # vpp model的layer间执行顺序与pp model不同，这里需要计算索引按实际执行顺序排列layer
                    total_layer_num = args.target_pipeline_parallel_size * vpp_rank * args.num_layers_per_virtual_pipeline_stage + pp_rank * args.num_layers_per_virtual_pipeline_stage + layer_id
                ...
                for ep_rank in range(args.target_expert_parallel_size):
                    for tp_rank in range(args.target_tensor_parallel_size):
                        ...
                        if args.num_layers_per_virtual_pipeline_stage and args.save_model_type == 'megatron':
                            layer_chunk = models[ep_rank][tp_rank].language_model.encoder.layers[
                                vpp_rank * args.num_layers_per_virtual_pipeline_stage + layer_id]
                        else:
                            layer_chunk = models[ep_rank][tp_rank].language_model.encoder.layers[layer_id]
                ...
                if args.save_model_type == 'megatron':
                    ...
                    if args.num_layers_per_virtual_pipeline_stage:
                        ...
                        for vp_rank in range(vp_size):
                            model = copy.deepcopy(models[ep_rank][tp_rank])
