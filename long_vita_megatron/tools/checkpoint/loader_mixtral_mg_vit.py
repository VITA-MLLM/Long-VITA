...
def _load_checkpoint(queue, args):
    ...
    margs.first_pipeline_num_layers = checkpoint_args.first_pipeline_num_layers
    margs.vit_load = checkpoint_args.vit_load
    margs.vision_seq_length = checkpoint_args.vision_seq_length
    margs.image_size = checkpoint_args.image_size
    ...
    # EVA ViT
    message = {
        "word embeddings": models[0][0].language_model.external_feature_model.vit.embedding.word_embeddings.weight.data,
        "conv1 weight": models[0][0].language_model.external_feature_model.vit.conv1.weight.data,
        "conv1 bias": models[0][0].language_model.external_feature_model.vit.conv1.bias.data,
        # "proj weight": models[0][0].language_model.external_feature_model.linear_proj.weight.data,
        # "proj bias": models[0][0].language_model.external_feature_model.linear_proj.bias.data,
        "position embeddings": models[0][0].language_model.external_feature_model.vit.embedding.position_embeddings.weight.data
    }

    queue_put("vit embeddings", message)

    message = {}
    if margs.vision_projector_type == "affine":
        message["proj weight"] = models[0][0].language_model.external_feature_model.linear_proj.weight.data
        message["proj bias"] =  models[0][0].language_model.external_feature_model.linear_proj.bias.data

    if margs.vision_projector_type == "mlp":
        fc1_weight = []
        fc2_weight = []
        for ep_rank in range(1):
            for tp_rank in range(tp_size):
                fc1_weight.append(models[ep_rank][tp_rank].language_model.external_feature_model.vision_projection.encoder.linear_fc1.weight.data)
                fc2_weight.append(models[ep_rank][tp_rank].language_model.external_feature_model.vision_projection.encoder.linear_fc2.weight.data)

        message[f"fc1 weight"] = torch.cat(fc1_weight, dim=0)
        message[f"fc2 weight"] = torch.cat(fc2_weight, dim=1)

    queue_put("vision projection", message)

    print("=" * 100)
    print([models[0][tp_rank].language_model.external_feature_model.vit.embedding.word_embeddings.weight.data.sum() for tp_rank in range(tp_size)])
    assert models[0][0].language_model.external_feature_model.vit.embedding.word_embeddings.weight.data.sum() == models[-1][-1].language_model.external_feature_model.vit.embedding.word_embeddings.weight.data.sum()
    print("=" * 100)


    total_layer_num = 0
    for vp_rank in range(1):
        ...
        for pp_rank in range(1):
            ...
            for layer_num, _ in enumerate(models[0][0].language_model.external_feature_model.vit.encoder.layers):
                message = {}

                # Get non-parallel tensors from tp_rank 0
                layer = models[0][0].language_model.external_feature_model.vit.encoder.layers[layer_num]
                ...
                message["mlp fc2 bias"] = layer.mlp.linear_fc2.bias.data
                ...
                print("=" * 100)
                assert models[0][0].language_model.external_feature_model.vit.encoder.layers[layer_num].mlp.linear_fc1.weight.data.sum() == models[-1][0].language_model.external_feature_model.vit.encoder.layers[layer_num].mlp.linear_fc1.weight.data.sum()
                assert models[0][0].language_model.external_feature_model.vit.encoder.layers[layer_num].self_attention.linear_qkv.weight.data.sum() == models[-1][0].language_model.external_feature_model.vit.encoder.layers[layer_num].self_attention.linear_qkv.weight.data.sum()
                print("=" * 100)
                for ep_rank in range(1):
                    for tp_rank in range(tp_size):
                        layer = models[ep_rank][tp_rank].language_model.external_feature_model.vit.encoder.layers[layer_num]
                        ...
                        fc1_weight.append(layer.mlp.linear_fc1.weight.data)
                        fc2_weight.append(layer.mlp.linear_fc2.weight.data)

                        fc1_bias.append(layer.mlp.linear_fc1.bias.data)
                ...
                message[f"mlp fc1 weight"] = torch.cat(fc1_weight, dim=0)
                message[f"mlp fc2 weight"] = torch.cat(fc2_weight, dim=1)

                message[f"mlp fc1 bias"] = torch.cat(fc1_bias, dim=0)

                queue_put(f"vit transformer layer {total_layer_num}", message)
