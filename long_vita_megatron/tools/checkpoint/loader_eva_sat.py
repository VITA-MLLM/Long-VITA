...
def add_arguments(parser):
    group = parser.add_argument_group(title='Eva2Clip SAT loader.')
...
def load_args_from_checkpoint(args):
    ...
    args.ffn_hidden_size = llama_args["inner_hidden_size"]
    args.patch_dim = llama_args["patch_size"]
    args.img_h = llama_args["image_size"][0]
    args.img_w = llama_args["image_size"][1]
...
def set_preprocess_state(args, model, hf_model):
    ...
    model.language_model.embedding.word_embeddings.weight.data.copy_(
        hf_model.transformer.word_embeddings.weight)
    model.language_model.embedding.position_embeddings.weight.data.copy_(
        hf_model.transformer.position_embeddings.weight
    )
    model.conv1 = hf_model.mixins["patch_embedding"].proj
...
def set_attn_state(args, layer, hf_layer):
    ...
    q_proj, k_proj, v_proj = hf_attn.query_key_value.weight.chunk(3, dim=0)
    q_bias, k_bias, v_bias = hf_attn.query_key_value.bias.chunk(3, dim=0)
...
def set_mlp_state(args, layer, hf_layer):
    ...
    mlp.dense_h_to_4h.bias.data.copy_(hf_mlp.dense_h_to_4h.bias)
    mlp.dense_4h_to_h.weight.data.copy_(hf_mlp.dense_4h_to_h.weight)
    mlp.dense_4h_to_h.bias.data.copy_(hf_mlp.dense_4h_to_h.bias)
...
def set_layer_state(args, model, hf_model, layer_idx):
    ...
    hf_layer = hf_model.transformer.layers[layer_idx]
    ...
    layer.input_norm.bias.data.copy_(hf_layer.input_layernorm.bias)
    layer.post_attention_norm.weight.data.copy_(hf_layer.post_attention_layernorm.weight)
    layer.post_attention_norm.bias.data.copy_(hf_layer.post_attention_layernorm.bias)
...
def _load_checkpoint(queue, args):
    ...
    assert args.model_type == 'EVA', 'Load eva2clip model.'
    ...
    message = {
        ...
        "conv weight": model.conv1.weight.data,
        "conv bias": model.conv1.bias.data
    }
